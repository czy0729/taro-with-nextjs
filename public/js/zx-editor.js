...
;(function(global, factory) {
  ...
  function changeNodeName(oldNode, tagName) {
    ...
    if (typeof tagName !== 'string' || util.strip(tagName).length === 0) {
      throw new TypeError(
        "changeNodeName(oldNode, tagName) 'tagName' is ".concat(
          tagName,
          ", should be a 'string'"
        )
      )
  ...
  function ZxQuery(selector, context) {
    ...
    if (typeof selector === 'string') {
      if (
        ...
        context =
          util.isElement(context) && context.nodeType === 1 ? context : doc
  ...
  ZxQuery.prototype = {
    ...
    every: function every(fn) {
      if (typeof fn !== 'function')
        throw new TypeError('every(fn) fn is not a function')
      var len = this.length
  ...
  function ExpansionPanel(options, zxEditor) {
    ...
    if (!zxEditor instanceof ZxEditor) {
      throw new TypeError(
        'new ExpansionPanel() parameter error, arguments[1] is not a ZxEditor instance.'
      )
    } // options
    ...
    this.name = opts.name
    this.$body = $(
      '<div class="body-wrapper" style="height:'.concat(
        opts.height - opts.headHeight,
        'px;"></div>'
      )
    ) // node
    ...
    if (opts.headHeight > 0) {
      ...
      if (opts.head) {
        ...
        // default head
        this.$head = $(
          '<div class="head-wrapper border-bottom '
            .concat(opts.headAlign, '" style="height:')
            .concat(opts.headHeight, 'px;line-height:')
            .concat(opts.headHeight, 'px;"><div class="l cur ')
            .concat(opts.headLeftBtnClassName, '">')
            .concat(opts.headLeftBtnText, '</div>')
            .concat(opts.headTitle || '', '</div>')
        ) // left btn
        ...
        $leftBtn.on(
          'click',
          zxEditor.$eventHandlers[this.name + 'HeadLeftBtn'].handler
        )
        var $switch = $(
          '<i class="switch" style="width:'
            .concat(opts.headHeight, 'px;height:')
            .concat(opts.headHeight, 'px;"></i>')
        )
        ...
        $switch.on(
          'click',
          zxEditor.$eventHandlers[this.name + 'HeadSwitch'].handler
        )
      }
  ...
  function styleExpansionPanel(options) {
    ...
    var COLORS = Array.isArray(options.textStyleColors)
      ? options.textStyleColors
      : DEF_COLORS
    ...
    function handleHeadClick(type) {
      ...
      if (type === 'left-button') {
        ...
        if (currentNode.nodeName !== 'SECTION') {
          zxEditor.$cursorNode = $(
            zxEditor.changeNodeName(currentNode, 'section')
          )
  ...
  function selectPictureBtn() {
    ...
    var $selectPictureLabel = $(
      '<label class="toolbar-icon-pic" style="position:absolute;top:0;left:0;width:100%;height:100%;"></label>'
    )
    var $selectPictrueInput = $(
    ...
    var imageSectionTemplate = /^<\w+\b.*<\/\w+>$/.test(
      options.imageSectionTemp
    )
      ? options.imageSectionTemp
      : '<section><img src="{url}"></section>' // register selectPictureInputChange
    ...
    this.$eventHandlers['selectPictureInputChange'] = {
      ...
      handler: function handler(e) {
        ...
        _this
          .fileToBase64(file, imageOptions)
          .then(function(res) {
            // console.log(res)
  ...
  function Toolbar(options, zxEditor) {
    ...
    this.$el = $(
      '<div class="zx-editor-toolbar-wrapper border-top '
        .concat(this.visible ? 'in' : 'out', '" style="height:')
        .concat(
          this.height + (util.isIPhoneX ? IPHONEX_BOTTOM_OFFSET_HEIGHT : 0),
          'px;"><dl class="inner-wrapper" style="height:'
        )
        .concat(this.height, 'px;"></dl></div>')
    ) // append to $editor
  ...
  Toolbar.prototype = {
    ...
    addButton: function addButton(opts, index) {
      ...
      var $btn = $(
        '<dd class="icon-item" data-name="'
          .concat(params.name, '" style="')
          .concat(params.style, '"></dd>')
      ) // handle el
      ...
      if (params.events) {
        ...
        events.forEach(function(item) {
          if (
            ...
            $btn.on(
              item.type,
              $eventHandlers[eventHandlerKey].handler,
              $eventHandlers[eventHandlerKey].capture
            )
          } else {
            throw new TypeError(
              "Function addButton(opts), opts.events's parameter error."
            )
          }
        })
      }
  ...
  function initDom(options) {
    ...
    // content
    this.$content = $(
      '<div class="zx-editor-content-wrapper is-empty" contenteditable="'.concat(
        options.editable,
  ...
  function initStyle(options) {
     ...
     */
    // main color
    var style = '.zx-editor .m-color{color:'
      .concat(
        options.mainColor,
        ' !important;}.zx-editor .text-style-outer-wrapper .__tag-wrapper dd i {border-color:'
      )
      .concat(options.mainColor, ' !important;}') // line-height, caret-color

    if (options.lineHeight || options.cursorColor || options.textColor) {
      var lineHeight = options.lineHeight
        ? 'line-height:'.concat(options.lineHeight, ';')
        : ''
      var caretColor = options.cursorColor
        ? 'caret-color:'.concat(options.cursorColor, ';')
        : ''
      var textColor = options.textColor
        ? 'color:'.concat(options.textColor, ';')
        : ''
      style += '.zx-editor .zx-editor-content-wrapper{'.concat(
        lineHeight + caretColor + textColor,
        '}'
      )
    }

    if (options.paragraphTailSpacing) {
      style += '.zx-editor .zx-editor-content-wrapper > *{margin-bottom:'.concat(
        options.paragraphTailSpacing,
        ';}'
      )
    } // placeholder

    if (options.placeholder || options.placeholderColor) {
      var content = options.placeholder
        ? "content:'".concat(options.placeholder, "';")
        : ''
      var color = options.placeholderColor
        ? 'color:'.concat(options.placeholderColor, ';')
        : ''
      style += '.zx-editor .zx-editor-content-wrapper.is-empty:before{'.concat(
        content + color,
        '}'
      )
    }
    ...
    if (options.borderColor) {
      style += '.zx-editor .border-top:before, .zx-editor .border-bottom:after {border-color: '.concat(
        options.borderColor
      )
    }
    /**
  ...
  function getImageInfo(base64, file, opts, callback) {
    ...
    img.onload = function() {
      ...
      callback(
        null,
        opts.orientation > 1
          ? rotateAndToBase64(defaultData, opts.orientation)
          : defaultData
      )
    }
  ...
  function base64ToBlobData(base64Data) {
    ...
    if (/^data:(\w+\/\w+);base64,(.+)/.test(base64Data)) {
      ...
      throw new TypeError(
        'base64ToBlobData(base64Data), base64Data non-base64 data!'
      )
    }
        ...
        getImageInfo(this.result, file, opts, function(e, raw) {
          ...
          if (opts.imageMaxSize > 0 && result.size / 1024 > opts.imageMaxSize) {
            reject(
              new RangeError(
                'File size "'
                  .concat(result.size / 1024, '" out of range, limit is ')
                  .concat(opts.imageMaxSize, 'Kib')
              )
            )
            return
          }
  ...
  var DEF_OPTIONS$1 = {
    ...
    // template
    imageSectionTemp:
      '<section class="child-is-picture"><img src="{url}"></section>',
  ...
  function ZxEditor(selector, _options) {
    if (!this instanceof ZxEditor) {
      throw new Error(
        'ZxEditor is a constructor and should be called with the `new` keyword'
      )
    }
    /**
     * ***************************************************
  ...
  ZxEditor.prototype = {
    ...
    insertElm: function insertElm(el) {
      ...
      if (typeof el === 'string') {
        ...
        if ($cursorNode.isEmpty()) {
          ...
          $cursorNode.children().every(function($item) {
          ...
          var tmpStr =
            rangeNodeStr.substr(0, rangeOffset) +
            el +
            rangeNodeStr.substr(rangeOffset) // $section = $cursorNode.closest('section')
    ...
    setHtml: function setHtml(html) {
      ...
      // @add set完blur
      setTimeout(() => {
        document.querySelector('.zx-editor-content-wrapper').blur()
      }, 100)
    },
    ...
    checkPosition: function checkPosition() {
      ...
      var line =
        height > this.lineHeight
          ? Math.floor(cursorOffset / textNumOfPerLine)
          : 1 // 当前光标位置，至当前元素顶部高度 - 1行高，防止移动后，光标位置太贴近顶部
      ...
      if (this.options.fixed);
        ...
        $(window).scrollTop(
          scrollTop + top + cursorHeight - this.options.cursorOffsetTop
        )
      }
    }
  }
